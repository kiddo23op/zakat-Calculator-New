  document.addEventListener('DOMContentLoaded', function() {
            ...
            calculateButton.addEventListener('click', function() {
                ...
                netWorth += parseFloat(document.getElementById('cashForInvestments').value) || 0;
                ...
                const taxBillsRent = parseFloat(document.getElementById('taxBillsRent').value) || 0;
                const employeeSalaries = parseFloat(document.getElementById('employeeSalaries').value) || 0;
                const borrowedMoney = parseFloat(document.getElementById('borrowedMoney').value) || 0;
                ...
                if (netWorth < nisabInput.value) {
                    showModal(`Your net worth is ${netWorth.toFixed(2)} ${currencyInput.value}. You're NOT eligible for giving Zakat.`);
                } else {
                    // Calculate Zakat amount
                    const zakatAmount = netWorth * 0.025; // Zakat is 2.5% of net worth
                    showModal(`Your net worth is ${netWorth.toFixed(2)} ${currencyInput.value}. Your payable Zakat amount is ${zakatAmount.toFixed(2)} ${currencyInput.value}.`);
                }
            });
